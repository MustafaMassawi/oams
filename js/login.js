...
function processLogin() {
  $("#login1").submit(function() {
    var data = {};
    $("#login1 input").each(function(k,v) {
      if(!$(v).val().length) {
      ...
      success : function(r) {
        ...
        switch(r.error) {
          ...
          case 'not_found' :
            ...
            $('.alert').removeClass('hidden');
            $("form#signup1 input[name=email]").val($("form#login1 input[name=email]").val());
            $("form#signup1 input[name=email]").focus();
            break;
          case 'incorrect' :
...
function processSignup() {
  $("#signup1").submit(function() {
    var data = {};
    var isEmpty = 0;
    $("#signup1 input").each(function(k,v) {
      if(!$(v).val().length) {
        $('.alert span').html('Please enter <strong>' + $(v).attr('name') + '</strong> !');
        $('.alert').removeClass('hidden');
        isEmpty++;
    ...
    if($("#signup1 input[name=password]").val() != $("#signup1 input[name=password2]").val()) {
      $('.alert span').html('Password don\'t match!');
      $('.alert').removeClass('hidden');
      return false;
    }
    if($("#signup1 input[name=password]").val().length < 6) {
    ...
    $.ajax({
      ...
      success : function(r) {
        ...
        switch(r.error) {
          ...
          break;
          case 'mismatch' : 
            $('.alert span').html('Password don\t match!');  
            $('.alert').removeClass('hidden');
          break;
          case 'small' : 
          ...
          case 'exists' : 
            ...
            $('.alert').removeClass('hidden');
            $("form#login1 input[name=email]").val($("form#signup1 input[name=email]").val());
            $("form#login1 input[name=email]").focus();
          break;
          case 'db_error' : 
